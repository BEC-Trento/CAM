...
def read(filename):
    ...
    '''
    filesizeNOW = os.path.getsize(filename)
    ...
    while(not os.path.getsize(filename) == filesizeNOW):
        filesizeNOW = os.path.getsize(filename)
        time.sleep(1)
    ...
    print "file {} shape {}".format(filename,rawdata.shape)
    # Reading the images
### Changed on 08/05/18: the old causes indefinite behaviour
